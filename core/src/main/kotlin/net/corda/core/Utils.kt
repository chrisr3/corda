...
/**
 * Returns a [ListenableFuture] bound to the single item emitted by this Observable. The future will complete with a
 * NoSuchElementException if no items are emitted or a IllegalArgumentException if more than one item is emitted. It will
 * also complete with any other error thrown by the Observable.
 */
